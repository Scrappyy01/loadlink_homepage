...
const navItems = [
  {
    ...
    href: '#',
    children: [
      { label: 'Parcel Freight', href: 'https://parcelfreight.loadlink.com.au/parcel-freight', description: 'Compare rates and book parcel delivery' },
      { label: 'LoadLink Connect', href: 'https://portal.loadlink.com.au/register/', description: 'Quote your freight for free and get it moving' },
      { label: 'Carriers Loadboard', href: 'https://app.loadlink.com.au/listings/', description: 'Find freight to haul near you' },
      { label: 'Tracking', href: '/#tracking', description: 'Real-time shipment tracking' },
    ],
  },
  {
    label: 'Integrations',
    ...
    <>
      ...
      >
        <div className="flex justify-between items-center px-[5%] md:px-[8%]">
          <Link href="/" className="relative z-10 hover:opacity-80 transition-opacity" aria-label="LoadLink — Go to homepage">
            <Image
      ...
      >
        <nav className="flex flex-col p-8 pt-24 gap-1" aria-label="Mobile navigation">
          {navItems.map((item) =>
            item.children ? (
              <div key={item.label}>
                <button
